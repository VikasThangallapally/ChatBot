...
export default function ResultPanel({result}){
  ...
  // Handle invalid image - Show clear error message
  if ((result.status === 'invalid_image') || (result.is_valid_brain_image === false)) {
    return (
      <motion.div 
        initial={{ y: 20, opacity: 0 }} 
        animate={{ y: 0, opacity: 1 }} 
        transition={{ duration: 0.6 }} 
        className="p-6 rounded-2xl bg-[rgba(239,68,68,0.1)] backdrop-blur-lg border-2 border-red-500/60 shadow-2xl hover:shadow-red-500/30"
      >
        <div className="flex items-center gap-3 mb-4">
          <div className="text-4xl">❌</div>
          <div>
            <h4 className="text-2xl font-bold text-red-400">Invalid Image</h4>
            <p className="text-sm text-red-300">Not a valid brain MRI scan</p>
          </div>
        </div>
        ...
        <div className="bg-red-500/20 border border-red-500/40 rounded-lg p-4 mb-4">
          <p className="text-red-200 font-semibold text-base mb-2">
            ⚠️ {result.validation_reason || result.error || 'This image does not match brain MRI characteristics.'}
          </p>
        </div>

        <div className="space-y-3">
          <div className="bg-red-500/15 border border-red-500/30 rounded p-4">
            <p className="text-red-200 font-semibold text-sm mb-2">❌ Examples of Rejected Images:</p>
            <ul className="text-red-200 text-xs space-y-1 ml-3">
              <li>• Photographs of people</li>
              <li>• Colored/natural images</li>
              <li>• Medical scans from other body parts</li>
              <li>• Blurry or low contrast images</li>
              <li>• Simple or blank images</li>
            </ul>
          </div>

          <div className="bg-green-500/15 border border-green-500/30 rounded p-4">
            <p className="text-green-200 font-semibold text-sm mb-2">✅ What You Should Upload:</p>
            <ul className="text-green-200 text-xs space-y-1 ml-3">
              <li>• Brain MRI scans (DICOM, JPEG, PNG)</li>
              <li>• Grayscale medical imaging</li>
              <li>• High contrast, clear images</li>
              <li>• Square or rectangle format</li>
              <li>• Minimum 100x100 pixels</li>
            </ul>
          </div>
        </div>

        <div className="mt-4 p-3 bg-yellow-500/15 border border-yellow-500/30 rounded">
          <p className="text-yellow-300 text-xs leading-relaxed">
            💡 Make sure you're uploading an actual brain MRI scan image, not a regular photograph.
          </p>
        </div>
      </motion.div>
    )
