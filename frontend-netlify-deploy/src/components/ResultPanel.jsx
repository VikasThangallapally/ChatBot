...
export default function ResultPanel({result}){
  ...
  if (result.status === 'invalid_image') {
    return (
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6 }} className="p-6 rounded-2xl bg-[rgba(255,255,255,0.02)] border border-red-500/20 shadow-neon">
        <h4 className="text-lg font-semibold mb-4 text-red-400">❌ Invalid Image</h4>
        <p className="text-slate-300 mb-4">{result.error || 'The uploaded file is not a valid brain MRI image.'}</p>
        <div className="bg-red-500/10 border border-red-500/20 rounded p-4 text-sm text-red-200">
          <strong>Requirements:</strong>
          <ul className="mt-2 space-y-1">
            <li>• Must be a brain MRI scan image</li>
            <li>• Minimum size: 150x150 pixels</li>
            <li>• Supported formats: JPG, PNG</li>
            <li>• Image must have sufficient contrast</li>
          </ul>
        </div>
      </motion.div>
    )
  ...
  // Handle success with full analysis
  if (result.status === 'success' && result.top_prediction && result.medical_analysis) {
